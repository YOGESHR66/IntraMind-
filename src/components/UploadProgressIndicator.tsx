...
export const UploadProgressIndicator: React.FC<UploadProgressIndicatorProps> = ({
  ...
  if (variant === 'compact') {
    return (
      ...
      >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between text-[11px] font-semibold mb-1">
              <span className="truncate max-w-[140px] sm:max-w-[220px]">
                {isError ? 'Upload notice' : isAborting ? 'Aborting upload...' : fileName || 'Document'}
              </span>
              {!isError && (
        ...
        {/* Action Button: Dismiss if error, Abort if in-progress */}
        {stage !== 'complete' && (
          <button
            type="button"
            onClick={(e) => {
            ...
            title={isError ? "Dismiss upload notice" : "Abort long-running upload"}
          >
            {isError ? <X className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
            <span>{isError ? 'Dismiss' : 'Abort'}</span>
          </button>
        )}
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 right-0 -mr-16 -mt-16 w-48 h-48 rounded-full blur-3xl pointer-events-none ${isError ? 'bg-rose-500/10' : 'bg-indigo-500/10'}`} />
      ...
      <div className="flex items-center justify-between gap-3 mb-3.5 relative z-10">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <h4 className="font-bold text-sm truncate flex items-center gap-2">
              <span>{isError ? 'Upload Notice' : isAborting ? 'Aborting Upload...' : fileName || 'Document'}</span>
              {totalChunks !== undefined && totalChunks > 0 && !isError && (
        ...
        {stage !== 'complete' && (
          <button
            ...
            title={isError ? "Dismiss notice" : "Abort upload"}
          >
            {isError ? <X className="w-3.5 h-3.5 text-rose-400" /> : <StopCircle className="w-3.5 h-3.5 text-rose-400" />}
            <span>{isError ? 'Dismiss' : 'Abort Upload'}</span>
          </button>
        )}
      </div>

      {/* Progress Bar Container (only when not in error) */}
      {!isError ? (
      ...
      {/* 5-Step Visual Pipeline Tracker (only when not in error) */}
      {!isError ? (
